  ...
  Future<bool> fetchUserInfos() async {
    ...
    try {
      final http.Response response =
          await http.get("https://foodie-a4fc5.firebaseio.com/users.json");
  ...
  Future<bool> addUserInfo(Map<String, dynamic> userInfo) async {
    ...
    try {
      final http.Response response = await http.post(
          "https://foodie-a4fc5.firebaseio.com/users.json",
    ...
    try {
      ...
      if (authMode == AuthMode.SignUp) {
        response = await http.post(
          "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyBemr1HzvmIih-VhQ_tbrGjX8rXjhfcWiU",
          body: json.encode(authData),
          headers: {'Content-Type': 'application/json'},
        );
      } else if (authMode == AuthMode.SignIn) {
        response = await http.post(
          "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyBemr1HzvmIih-VhQ_tbrGjX8rXjhfcWiU",
          body: json.encode(authData),
          headers: {'Content-Type': 'application/json'},
        );
      }
